...
public class AddressController {
    ...
    @ApiOperation(value = "查询用户id查询收货地址列表", notes = "查询用户id查询收货地址列表", httpMethod = "POST")
    @PostMapping("/list")
    ...
    @PostMapping("/add")
    public IMOOCJSONResult add(@ApiParam(name = "addressBO", value = "实体BO", required = true)
    ...
    @PostMapping("/update")
    public IMOOCJSONResult update(@ApiParam(name = "addressBO", value = "实体BO", required = true)
                                  @RequestBody AddressBO addressBO) {
    ...
    public IMOOCJSONResult setDefalut(@ApiParam(name = "userId", value = "用户id", required = true)
        ...
        if (StringUtils.isBlank(addressId) || StringUtils.isBlank(userId)) {
            return IMOOCJSONResult.errorMsg("设置默认地错误：userId、addressId不能为空");
        }
