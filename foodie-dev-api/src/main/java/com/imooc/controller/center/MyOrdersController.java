...
public class MyOrdersController extends BaseController {
    ...
    public IMOOCJSONResult query(
        ...
        if (StringUtils.isBlank(userId)) {
            IMOOCJSONResult.errorMsg("用户Id不存在");
        }
        if (page == null) {
            page = 1;
        }
    ...
    /**
     * 用户确认收货、用户删除订单接口代码有重复部分，不建议合并为一个接口
     * 虽然合并接口代码会得到共路径，但是代码的耦合度降低了，
     * 后续如果各自的逻辑业务增加，会导致代码越来越多，耦合度越来越大
     */
     ...
     * @return
     */
    @ApiOperation(value = "获取订单状态数量概况", notes = "获取订单状态数量概况", httpMethod = "POST")
    @PostMapping("/statusCounts")
    public IMOOCJSONResult statusCounts(
            @ApiParam(name = "userId", value = "用户Id", required = true)
            @RequestParam String userId) {

        OrderStatusCountsVO orderStatusCounts = myOrdersService.getMyOrderStatusCounts(userId);
